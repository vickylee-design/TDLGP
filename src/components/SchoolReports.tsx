...
// --- Mock Data ---
const GROUP_OPTIONS = [
  '三年二班', '三年五班', '四年一班', '電腦教室(一)', '電腦教室(二)', '圖書館', '教師辦公室'
];
...
const MOCK_DEVICES: DeviceData[] = Array.from({ length: 40 }).map((_, i) => ({
  id: `dev-${i}`,
  ...
  group: GROUP_OPTIONS[i % GROUP_OPTIONS.length],
...
const SchoolReports: React.FC = () => {
  ...
  const baseFilteredDevices = useMemo(() => {
    return MOCK_DEVICES.filter(device => {
      const groupMatch = appliedFilters.groups.length === 0 || appliedFilters.groups.includes(device.group);
      const serialMatch = !appliedFilters.serialNumber || device.serial.toLowerCase().includes(appliedFilters.serialNumber.toLowerCase());
      return groupMatch && serialMatch;
    });
  }, [appliedFilters]);
  ...
  const stats = useMemo(() => {
    ...
    // Group breakdown
    const groupBreakdown = appliedFilters.groups.length > 0 
      ? appliedFilters.groups.map(groupName => {
          const groupDevices = baseFilteredDevices.filter(d => d.group === groupName);
          return {
            name: groupName,
    ...
    return { total, used, unused, usageRate, groupBreakdown };
  }, [baseFilteredDevices, appliedFilters.groups]);
  ...
  const exportToExcel = () => {
    ...
    const cardData = [
      ...
      [''],
      ['各群組分佈'],
      ...stats.groupBreakdown.map(s => [s.name, s.count]),
      ['']
    ];

    // 2. Prepare List Data
    const listHeader = ['設備名稱', '設備序號', '設備型號', '指派群組', '目前 MDM', '使用時長', '最後連線時間'];
    ...
    // 5. Download
    XLSX.writeFile(wb, `校內報表_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const getFilterSummary = () => {
    ...
    if (appliedFilters.groups.length > 0) {
      parts.push(`群組: ${appliedFilters.groups.join(', ')}`);
    } else {
      parts.push('群組: 全部');
    }
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto p-2">
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">報表</h2>
            <p className="text-slate-500 text-sm">校內設備使用狀況分析與數據導出</p>
          </div>
        </div>
        <button 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="space-y-4 border-r border-slate-100 pr-8">
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-bold text-slate-700">群組選擇</label>
                      <button 
                        onClick={() => setFilters(f => ({ ...f, groups: [] }))}
                    ...
                    <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1">
                      {GROUP_OPTIONS.filter(s => s.includes(groupSearch)).map(group => {
